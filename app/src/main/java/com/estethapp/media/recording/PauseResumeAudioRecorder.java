...
public class PauseResumeAudioRecorder {
    ...
    public void startRecording() {
        if (currentAudioState.get() == PREPARED_STATE && isSuppported) {
            ...
            int b = MediaRecorder.AudioEncoder.AMR_WB;
            currentAudioRecordingThread = new AudioRecorderThread(audioFile.replace(".mp3",
              ".temp"), a, sampleRateInHertz, channelConfig, b, maxFileSizeInBytes);
    ...
    private class AudioRecorderThread extends Thread {
         ...
         */
        AudioRecorderThread(final String threadAudioFile, final int threadAudioSource,
                            final int threadSampleRateHertz, final int threadChannelConfig, final int threadAudioEncoding, final long maxFileSizeInBytes) {
                ...
                try {
                    ...
                    try {
                        if (dataOutputStream != null) {
                            ...
                            if (!(new File(threadAudioFile).renameTo(new File(threadAudioFile.replace(".temp", ".mp3"))))) {
