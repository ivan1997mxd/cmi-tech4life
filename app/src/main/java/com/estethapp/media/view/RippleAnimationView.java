...
public class RippleAnimationView extends RelativeLayout {
    ...
    private void init(final Context context, final AttributeSet attrs) {
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.RippleAnimationCustom);

        //it will return ripple color even we have specified into xml or else it will get the default ripple Color
        rippleColor = typedArray.getColor(R.styleable.RippleAnimationCustom_rb_color, getResources().getColor(R.color.red));

        //getting rippleStrokeWidth from typedArray object and putting it into rippleStrokeWidth variable
        rippleStrokeWidth = typedArray.getDimension(R.styleable.RippleAnimationCustom_rb_strokeWidth, getResources().getDimension(R.dimen.rippleStrokeWidth));

        //rippleRadius contains the default the specified radius even we specified or else it takes default radius...which is 64dp
        rippleRadius = typedArray.getDimension(R.styleable.RippleAnimationCustom_rb_radius, getResources().getDimension(R.dimen.rippleRadius));

        //setting rippleAnimation duration is specified or else it takes default Duration Time.
        rippleDurationTime = typedArray.getInt(R.styleable.RippleAnimationCustom_rb_duration, DEFAULT_DURATION_TIME);

        //number of ripple if we have specified into design part or else default value it will take.
        rippleAmount = typedArray.getInt(R.styleable.RippleAnimationCustom_rb_rippleAmount, DEFAULT_RIPPLE_COUNT);

        //setting scale of ripple
        rippleScale = typedArray.getFloat(R.styleable.RippleAnimationCustom_rb_scale, DEFAULT_SCALE);

        //its getting the ripple type and putting it into rippleType variable.
        rippleType = typedArray.getInt(R.styleable.RippleAnimationCustom_rb_type, DEFAULT_FILL_TYPE);
